...
function clearEmailOrPhone(str: string, type: string) {
	...
	return type.includes('phone') ? str.replaceAll(/[^a-zA-Z0-9+]/g, '') : str.trim()
}
...
function reMapContactProps(info: ContactInfo) {
	return Object.entries(info).map((_item) => {
		...
		const _ifLocation = !!value && type.includes('location') && typeof value !== 'string' ? Object.values(value).join(', ') : ''
