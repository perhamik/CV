...
function isWorkItem(item: WorkExperience | EducationExperience): item is WorkExperience {
...
function ExperienceItem({item, index}: {item: WorkExperience; index: number}) {
	...
	const date = React.useMemo<string>(() => formateDate(item.startDate, item.endDate), [item.startDate, item.endDate])
		...
		<div key={title} className={style.container}>
			...
			<ul className={style.list} data-work={isWorkItem(scope[0])}>
				{scope.map((item, _id) => {
					...
					return <ExperienceItem key={_id} item={exp} index={indexBegin + _id + 1} />
				})}
			</ul>
		</div>
	)
...
export default function Work({work, education}: ExperienceProps) {
	return (
		<>
			...
			<Experience scope={education} title="Education" indexBegin={work.length} />
		</>
	)
}
